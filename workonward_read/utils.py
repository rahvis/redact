"""
Utility functions for WorkOnward Read.
"""
...
def get_package_dir():
    """Get the directory of the workonward_read package."""
...
def get_resource_root():
    ...
    The PyInstaller hook (hook-workonward_read.py) collects data files using
    collect_data_files(), which preserves the package structure.
    Files are placed under 'workonward_read/fonts/' in the bundle.
    """
    # PyInstaller >= 5.0: _MEIPASS contains extracted files
    if hasattr(sys, "_MEIPASS"):
        # Hook collects to 'workonward_read/fonts', so look there first
        pyinstaller_path = os.path.join(sys._MEIPASS, "workonward_read")
    ...
    if getattr(sys, "frozen", False):
        ...
        pkg_path = os.path.join(exe_dir, "_internal", "workonward_read")
...
def _find_resource_folder(root, target):
    """
    Find a named resource folder within the given root directory.

    Searches in order:
    1. Direct child folder
    2. Recursive search for any folder with that name
    """
    # First check if the folder is directly in the root (package directory)
    direct_path = os.path.join(root, target)
...
def find_fonts_folder(root):
    ...
    return _find_resource_folder(root, "fonts")


def find_assets_folder(root):
    """Find the assets folder (app icons etc.) within the given root directory."""
    return _find_resource_folder(root, "assets")
